...
def get_corpus(filename):
    ...
    # Canterbury
    if any(x in fname for x in ['alice', 'asyoulik', 'cp.html', 'fields', 
                                'grammar', 'kennedy', 'lcet', 'plrabn', 
                                'ptt5', 'sum', 'xargs']):
        return 'Canterbury'
    
    # Calgary
    calgary_files = ['bib', 'book1', 'book2', 'geo', 'news', 'obj1', 'obj2',
                     'paper1', 'paper2', 'paper3', 'paper4', 'paper5', 'paper6',
                     'pic', 'progc', 'progl', 'progp', 'trans']
    if any(fname == f or fname.startswith(f + '.') for f in calgary_files):
        return 'Calgary'
    
    # Large
    if any(x in fname for x in ['bible', 'e.coli', 'world192']):
        return 'Large'
    ...
    # Heterogeneous (mixed content)
    if 'mixed' in fname or 'heterogeneous' in fname:
...
print(f"Using {len(df)} rows after excluding artificial files")
...
ax.set_title('Compression Ratio Distribution (Real-World Files)\n(Green Line = Median)', 
...
plt.tight_layout()
plt.savefig('results/figures/figure1_compression_ratio_boxplots_300plus_files_without_memory_post_table1_fix.png', dpi=300, bbox_inches='tight')
print("✅ Saved: figure1_compression_ratio_boxplots_300plus_files_without_memory_post_table1_fix.png")

# --- START OF MODIFICATION ---

# Calculate Average Performance and Pareto Frontier (Required for Figure 2 and 4)
df_avg = df.groupby('algorithm').agg({
    'compression_ratio': 'median',  # ✅ MEDIAN
    'speed_mbps': 'mean',           # Mean OK for speed
    'memory_mb': 'mean'
}).reset_index()
...
plt.tight_layout()
plt.savefig('results/figures/pareto_ratio_speed_14_DEC_2025_300plus_files_without_memory_post_table1_fix.png', dpi=300, bbox_inches='tight')
print("✅ Saved: pareto_ratio_speed_14_DEC_2025_300plus_files_without_memory_post_table1_fix.png (Updated with N=391 data)")
...
corpus_algo = df.groupby(['corpus', 'algorithm'])['compression_ratio'].median().reset_index()
...
plt.tight_layout()
plt.savefig('results/figures/figure3_corpus_analysis_300plus_files_without_memory_post_table1_fix.png', dpi=300, bbox_inches='tight')
print("✅ Saved: figure3_corpus_analysis_300plus_files_without_memory_post_table1_fix.png")
...
df_parallel = df_norm[['algorithm', 'compression_ratio_norm', 
                       'speed_mbps_norm']].copy()

#df_parallel = df_norm[['algorithm', 'compression_ratio_norm', 
#                       'speed_mbps_norm', 'memory_mb_norm']].copy()
df_parallel.columns = ['algorithm', 'Ratio', 'Speed']
...
plt.tight_layout()
plt.savefig('results/figures/figure4_preference_tradeoffs_300plus_files_without_memory_post_table1_fix.png', dpi=300, bbox_inches='tight')
print("✅ Saved: figure4_preference_tradeoffs_300plus_files_without_memory_post_table1_fix.png")
